...
| App URL | `http://docsort.local` or `http://192.168.178.158` |
| Health | `GET /up` → **200** |
...
- `kamal-proxy` — HTTP front (hosts: IP, `docsort.local`, `docsort`)  
...
# F) Verify
curl -sS -m 10 -o /dev/null -w "up %{http_code}\n" http://docsort.local/up \
  || curl -sS -m 10 -o /dev/null -w "up-ip %{http_code}\n" http://192.168.178.158/up
```
